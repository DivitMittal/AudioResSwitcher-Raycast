...
class OutputFormatsCommand {
  ...
  private func handleSetFormat() {
    guard CommandLine.arguments.count >= 4 else {
      outputError("Usage: output-formats.swift set <sampleRate> <bitDepth> [channels]")
      return
    }
    ...
    guard let deviceID = getDefaultOutputDevice() else {
      outputError("Could not find default output device")
      return
    }
    ...
    if setFormat(deviceID: deviceID, sampleRate: sampleRate, bitDepth: bitDepth, channels: channels)
    {
      ...
      outputError("Failed to set output format")
    }
  }
  ...
  private func listFormats() {
    ...
    let currentFormat = getCurrentStreamFormat(deviceID: deviceID, isInput: false)
    ...
    for rate in availableRates {
      let streamFormats = getAvailableStreamFormats(
        deviceID: deviceID, sampleRate: rate, isInput: false)
    ...
    -> Bool
  {
    ...
    let streamFormats = getAvailableStreamFormats(
      deviceID: deviceID, sampleRate: sampleRate, isInput: false)
    ...
    if let format = targetFormat {
      return setStreamFormat(deviceID: deviceID, format: format, isInput: false)
    } else {
  ...
  // MARK: - Audio Core Functions (same as input-formats.swift)
...
let command = OutputFormatsCommand()
command.run()
